...
func LogMiddleware(next http.Handler) http.Handler {
	return http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		// 	LogErrorHTTP(msgLog.InternalID, err, "Error reading request body")
		...
		// 	LogErrorHTTP(msgLog.InternalID, err, models.ErrBadRequest.Error())
		// 	http.Error(rw, "Invalid JSON", http.StatusBadRequest)
		// 	return
		// }
		//
		// // Log the message with the request details (async logging)
		// go LogMsgInterfaceHTTP(*msgLog, &m)

		// Create a context with the message log
		ctx := context.WithValue(r.Context(), MessageLog{}, msgLog)
		...
		defer func() {
			if r := recover(); r != nil {
				LogErrorHTTP("SYSTEM", fmt.Errorf("recovered from panic: %v", r), "panic recovery")
